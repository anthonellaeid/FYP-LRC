...
//insert mission
// if(!empty($_POST["mission_type"]) && $_POST["mission_type"]=='5'){
    ...
    if($_POST["initial_mileage"]<$_POST["final_mileage"]){
        ...
        if($dropdownsrc=='' && $dropdowndest==''){
            if($srctxt!='' && $desttxt!=''){
                ...
                $resupdate=updateMission2($StartDate,$endDate,$initial_KM,$final_km,$etank,$dtank,$mtank,$missionType,$equipe,$ambId,$uhfRadio,$cmKit,$srcTxtAddress,$destTxtAddress);
            }
        }else{
            if($dropdownsrc!='' && $dropdowndest==''){
                if($desttxt!=''){
                    $resupdate=updateMission2($StartDate,$endDate,$initial_KM,$final_km,$etank,$dtank,$mtank,$missionType,$equipe,$ambId,$uhfRadio,$cmKit,$dropdownsrc,$maxidaddress);
                }
            }else{
                if($dropdownsrc=='' && $dropdowndest!=''){
                    if($srctxt!=''){
                        $maxidaddress++;
                        $resupdate=updateMission2($StartDate,$endDate,$initial_KM,$final_km,$etank,$dtank,$mtank,$missionType,$equipe,$ambId,$uhfRadio,$cmKit,$maxidaddress,$dropdowndest);
                    }
                }else{
                    if($dropdownsrc!='' && $dropdowndest!=''){
                        $resupdate=updateMission2($StartDate,$endDate,$initial_KM,$final_km,$etank,$dtank,$mtank,$missionType,$equipe,$ambId,$uhfRadio,$cmKit,$dropdownsrc,$dropdowndest);
                    }
                }
            }
        }
...
//insert into mission_rescuer
   ...
   if($resupdate && !empty($_POST["driver"]) ){
    ...
    if($resupdate && !empty($_POST["emt"])){
     ...
     // ↓ insert emt2 ↓
     if($resupdate && !empty($_POST["emt2"])){    
        if($EMT2==$_SESSION["rescuer_id"]){echo " this EMT:".$EMT2." is the creator***"; $emtRole=1;}
        else{$emtRole=0;}
        $x=insertintoMissionRescuer($EMT2,3,$emtRole);
        echo "<br>";
        if($x){
            echo "XX";
        }
    ...
    // insert emt3
    if($resupdate && !empty($_POST["emt3"])){    
        if($EMT3==$_SESSION["rescuer_id"]){echo " this EMT:".$_POST["emt3"]." is the creator***"; $emtRole=1;}
        else{$emtRole=0;}
        $y=insertintoMissionRescuer($EMT3,3,$emtRole);
        echo "<br>";
        if($y){
            echo "YY";
        }
