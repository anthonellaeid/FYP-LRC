...
function sortTabl($orders,$ordersBy){
    require("DBconnection.inc.php");
    $Todayz=date("Y-m-d");
    $sql = "SELECT * from today where missionStatus_code=2	and Iscreator=1 and Date(mission_start_date)='".$Todayz."' ORDER BY " . $ordersBy . " " . $orders;
...
function sortCanceled($orderCancel,$orderByCanceled){
    require("DBconnection.inc.php");
    $Todayz=date("Y-m-d");
    $sqlCancel = "SELECT * from today where missionStatus_code=3 and Iscreator=1 and Date(mission_start_date)='".$Todayz."' ORDER BY " . $orderByCanceled . " " . $orderCancel;
